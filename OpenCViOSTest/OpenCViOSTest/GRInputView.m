...
- (void)setup
{
    ...
    self.contentInset = UIEdgeInsetsMake(5.0f, 0.0f, 0.0f, 30.0f);
...
- (void)undoEvent:(UIButton *)sender {
...
- (UIButton *)undoButton {
    if (_undoButton == nil) {
        ...
        _undoButton.frame = CGRectMake(self.frame.size.width - 25, -5, 20, 20);
        [_undoButton setTitle:@"X" forState:UIControlStateNormal];
...
- (void)setMaxNumberOfLines:(NSUInteger)maxNumberOfLines
{
   ...
   CGFloat height = [self heightForString:_placeholderView andWidth:self.frame.size.width];
    // 计算最大高度 = (每行高度 * 总行数 + 文字上下间距)
    _maxTextH = ceil(height * maxNumberOfLines + 10);
